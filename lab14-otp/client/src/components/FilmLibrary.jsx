    ...
    <Table>
      ...
      <tbody>
        {films.map((film) => <FilmRow filmData={film} key={film.id} delete={props.delete}
           editFilm={props.editFilm} setFilmFavorite={props.setFilmFavorite} setFilmRating={props.setFilmRating}
    ...
    <tr>
      ...
      <td className="text-center">
        ...
        <Form.Check type="checkbox" checked={props.filmData.favorite ? true : false} 
          onChange={(event) => props.setFilmFavorite(props.filmData.id, event.target.checked) } />
      </td>
      <td>
      ...
      <td>
        <Rating rating={props.filmData.rating} maxStars={5} 
          editRating={(newRating) => { if (newRating!==props.filmData.rating) props.setFilmRating(props.filmData.id, newRating )} } />
      </td>
      <td>
        <Button variant='danger' disabled={props.disableActions}
          onClick={() => { props.delete(props.filmData.id) }} >
          <i className='bi bi-trash'></i>
        </Button>
        <Button className="mx-2" variant='warning' disabled={props.disableActions}
          onClick={() => { navigate(`/edit/${props.filmData.id}`) }} >
